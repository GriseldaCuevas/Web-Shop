var _scq = _scq || [];
_scq.push(['setMerchantId', '17A476D76959E253']);
'use strict';
    ...
    for (var i=0;i<vars.length;i++) {
      ...
      try {
        ...
        if (typeof queryString[pair[0]] === 'undefined') {
          queryString[pair[0]] = paramValue;
          // If second entry with this name
        } else if (typeof queryString[pair[0]] === 'string') {
          var arr = [ queryString[pair[0]], paramValue ];
          queryString[pair[0]] = arr;
          // If third or later entry with this name
        } else {
          queryString[pair[0]].push(paramValue);
        }
      } catch(err) {
...
k)-899497514);j=k;k=e;e=g<<30|g>>>2;g=h;h=c}b[0]=b[0]+h|0;b[1]=b[1]+g|0;b[2]=b[2]+e|0;b[3]=b[3]+k|0;b[4]=b[4]+j|0},_doFinalize:function(){var f=this._data,e=f.words,b=8*this._nDataBytes,h=8*f.sigBytes;e[h>>>5]|=128<<24-h%32;e[(h+64>>>9<<4)+14]=Math.floor(b/4294967296);e[(h+64>>>9<<4)+15]=b;f.sigBytes=4*e.length;this._process();return this._hash},clone:function(){var e=j.clone.call(this);e._hash=this._hash.clone();return e}});e.SHA1=j._createHelper(m);e.HmacSHA1=j._createHmacHelper(m)})();
  ...
  return {
    ...
    hookedSrc: staticResource('/hooked/scarab-hooked.js'),
    script: {},
...
var ScarabModule = function(win, config) {
    ...
    Transaction.prototype = {
        ...
        go : function(immediate, forceSend) {
            ...
            if (!testMode && this.id > 1 && Math.abs(transactions[getTrnName(this.id)].time - transactions[getTrnName(this.id-1)].time) < 500) {
...
(function() {
    ...
    try {
        var halfAnHourLater = new Date();
        halfAnHourLater.setMinutes(halfAnHourLater.getMinutes() + 30);
        if (window.location.hash.indexOf('sc_inspector') > -1 ) {
            document.cookie = 'sc_inspector=true;expires=' + halfAnHourLater + ';path=/';
        }
        ...
        if (window.location.hash.indexOf('sc_assistant') > -1) {
            document.cookie = 'sc_assistant=true;expires=' + halfAnHourLater + ';path=/';
        }
        if (window.location.hash.indexOf('sc_hooked') > -1) {
            document.cookie = 'sc_hooked=true;expires=' + halfAnHourLater + ';path=/';
        }
        if (ScarabUtil.getCookie('sc_hooked') === true) {
            ScarabUtil.loader(ScarabUtil.hookedSrc, 'scarab-hooked', function(hooked) {
                hooked.go();
            });
        }
    } catch(e) {
